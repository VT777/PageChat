...
def test_chat_stream_api_forwards_regenerate_boundary(
    tmp_path: Path,
    monkeypatch,
) -> None:
    db_path = tmp_path / "stream-regenerate-boundary.db"
    ...
    response = _client(db_path).post(
        ...
        json={
            ...
            "regenerate_from_message_id": "user-message-a",
        },
    )

    assert response.status_code == 200
    assert captured["regenerate_from_message_id"] == "user-message-a"
