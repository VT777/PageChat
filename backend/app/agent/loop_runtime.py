    ...
    def call_tool(
        ...
        *,
        ...
        return cls(
            ...
            tool_calls=(ToolCallRequest(tool_name, dict(arguments or {})),),
        )

    @classmethod
    def call_tools(
        cls,
        tool_calls: list[ToolCallRequest | tuple[str, dict[str, Any]] | dict[str, Any]],
        *,
        thought: str = "",
    ) -> "PlannerAction":
        ...
        for item in tool_calls:
            ...
            if not isinstance(arguments, dict):
                arguments = {}
            normalized.append(ToolCallRequest(str(tool_name), dict(arguments)))
        if not normalized:
            return cls(action_type="call_tool", thought=thought)
        first = normalized[0]
        return cls(
            action_type="call_tool",
            thought=thought,
...
class AgentLoopRuntime:
    ...
    async def stream(self, state: AgentRunState):
        ...
        for step in range(1, self.max_steps + 1):
            ...
            if action.action_type == "call_tool":
                executed_any = False
                async for event in self._execute_tool_actions(state, action, step):
                    executed_any = True
                    yield event
                if not executed_any and emitted_plan:
                    yield PageChatRuntimeEvent(
                        "progress",
                        {
                            "kind": "plan_retract",
    ...
    async def _execute_tool_actions(
        self,
        state: AgentRunState,
        ...
        for tool_action in self._tool_actions(action):
            if self.policy is not None:
                validation = self.policy.validate(tool_action, state)
                if not validation.allowed:
                    observation = dict(validation.observation or {})
                    observation.setdefault("kind", "guardrail")
                    observation.setdefault("step", step)
                    ...
                    continue
                if validation.action is not None:
                    tool_action = validation.action

            async for event in self._execute_tool_action(state, tool_action, step):
                yield event
            if callable(mark_tool_executed):
                mark_tool_executed(tool_action)

    def _tool_actions(self, action: PlannerAction) -> list[PlannerAction]:
