"""Expand flat visual TOC skeletons with per-page outline titles."""
...
_NOISE_PATTERNS = [
    ...
    re.compile(r"^第[一二三四五六七八九十\d]+\s*[章节篇部分]\s*$"),
]
...
def _make_child_node(parent: Dict, child_no: int, candidate: Dict, end_page: int) -> Dict:
    ...
    return {
        ...
        "end_index": end_page,
        "nodes": [],
        "source": candidate.get("source") or "visual_page_title",
...
def _is_preface(node: Dict) -> bool:
    ...
    if (
        ...
        return True
    return structure == "0" or "preface" in title or "前言" in title
