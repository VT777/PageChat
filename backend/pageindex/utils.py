...
def _get_sync_client():
    ...
    if _sync_client is None:
        _sync_client = OpenAI(api_key=_llm_api_key, base_url=_llm_base_url)
    return _sync_client
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    for i in range(max_retries):
        try:
            ...
            wait_seconds = min(2 ** i, 60)  # 指数退避，最大60秒
            print(f'************* Retrying ({i+1}/{max_retries}) after {wait_seconds}s *************')
...
def ChatGPT_API(model=None, prompt=None):
    return llm_completion(model=model, prompt=prompt)


def ChatGPT_API_with_finish_reason(model=None, prompt=None):
    return llm_completion(model=model, prompt=prompt, return_finish_reason=True)
...
def is_garbled_text(text, threshold=0.15):
    """Detect if text is garbled (too many replacement characters or non-printable chars)."""
    if not text:
        return False
    ...
    # Text is garbled if >threshold of chars are replacement or control chars
    garbled_ratio = (replacement_count + control_count) / total_chars
...
def dedup_tree(tree):
    """Remove children that have the same title as their parent."""
    for node in tree:
        if 'nodes' in node:
            # Filter out children with same title as parent
            original_children = node['nodes']
            node['nodes'] = [child for child in original_children 
                              if child.get('title') != node.get('title')]
            # If all children were duplicates, clean up
            if not node['nodes']:
                del node['nodes']
            else:
                # Recurse
                dedup_tree(node['nodes'])
    return tree

def add_preface_if_needed(data, page_list=None):
    if not isinstance(data, list) or not data:
        return data

    if data[0]['physical_index'] is not None and data[0]['physical_index'] > 1:
        # P2: Skip if Page 1 has very little content (<50 chars), likely cover page
        if page_list and len(page_list) > 0:
            page1_text = page_list[0][0] if page_list[0] else ""
            if len(page1_text.strip()) < 50:
                print("[ADD-PREFACE] Skipping preface: Page 1 is too short (likely cover page)")
                return data
...
def post_processing(structure, end_physical_index):
    ...
    for i, item in enumerate(structure):
        ...
        # P1: Guard against inverted range (start > end)
